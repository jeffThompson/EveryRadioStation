...
/*
  ...
  HARDWARE
  Setup for 8 radios per section, 12 of these setups total (the final
  section actually has only 7 radios, for a total of 95)
...
const int whichSection = 9;       // which section of radios? (0–11)
...
// backwards since I wired them wrong :(
...
const long startupDelay =  5600;          // millis to delay for each set to begin
...
void setup() {
  ...
  int startChannel = 879 + ((whichSection) * 16);
  ...
  if (whichSection == 11)  {
    numRadios = 7;              // final section only has 7
  }
  if (debug) {
    Serial.print("# radios:      ");
  ...
  if (delayStart) {
    long delayTime = whichSection * startupDelay;
    Serial.print("(Delaying startup ");
  ...
  for (int i=numRadios-1; i>=0; i-=1) {       // backwards, since I wired them wrong :(
